...
export function initMetrics() {
  ...
  return {
    ...
    mcu_info: new Gauge({
      ...
      labelNames: ['name', 'build_versions', 'version'],
    }),
    mcu_stats: {
      ...mcu_gauges,
    }
...
export async function emitStats(status: PrinterStatus, metrics: Metrics) {
  ...
  for (const mcuName of Object.keys(status.mcus)) {
    metrics.mcu_info.set({name: mcuName, build_versions: status.mcus[mcuName].build_versions, version: status.mcus[mcuName].version }, 1)
